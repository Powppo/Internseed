...
@section(Config::get('chatter.yields.head'))
	...
	<link href="/vendor/devdojo/chatter/assets/css/editDelete.css" rel="stylesheet">
@stop
...
<div id="chatter" class="discussion">
	...
	<div class="container margin-top" style="font-family: 'Poppins', sans-serif;">
						...
						<li data-id="{{ $post->id }}" data-markdown="{{ $post->markdown }}">
							<span class="chatter_posts">
								...
								<div id="delete_warning_{{ $post->id }}" class="chatter_warning_delete">
									...
									<button class="btn btn-sm pull-right delete_response">Yes Delete It</button>
								...
								<div class="chatter_post_actions">
									...
									<div class="option" style="padding-right: 5px;">
										<p class="chatter_delete_btn">
											<img style="width: 20px;" src="/vendor/devdojo/chatter/assets/images/delete.png" alt="">
										</p>
										<p class="chatter_edit_btn">
											<img style="width: 17px; margin-top: 1.5px" src="/vendor/devdojo/chatter/assets/images/edit.png" alt="">
										</p>
									</div>
								</div>
								@endif
								<div class="chatter_avatar">
									...
									@else
									<span class="chatter_avatar_circle" style="background-color:#x<?= \DevDojo\Chatter\Helpers\ChatterHelper::stringToColorCode($post->user->email) ?>">
								...
								<div class="chatter_middle">
									<span class="chatter_middle_details"><a href="{{ \DevDojo\Chatter\Helpers\ChatterHelper::userLink($post->user, 1) }}">{{ ucfirst($post->user->{Config::get('chatter.user.database_field_with_user_name')}) }}</a> <span style=" padding:10px; position: absolute; right: 0%; bottom: 0%; font-size: 10px; align-items: center;" class="ago chatter_middle_details">{{ \Carbon\Carbon::createFromTimeStamp(strtotime($post->created_at))->diffForHumans() }}</span></span>
	            ...
	            @if(!Auth::guest())
	            	...
	            	<div id="new_response">
	            		...
	            		<div class="chatter_avatar">
		        			...
		        			@else
		        				<span class="chatter_avatar_circle" style="background-color:#x<?= \DevDojo\Chatter\Helpers\ChatterHelper::stringToColorCode(Auth::user()->email) ?>">
...
<script>
	$('document').ready(function(){
		...
		$('.chatter_edit_btn').click(function(){
			...
			container.append('<div class="chatter_update_actions"><button class="btn pull-right update_chatter_edit"  data-id="' + id + '" data-markdown="' + markdown + '"><i class="chatter-check"></i> Update Response</button><button href="/" class="btn btn-default pull-right cancel_chatter_edit" data-id="' + id + '"  data-markdown="' + markdown + '">Cancel</button></div>');
...
<script src="/vendor/devdojo/chatter/assets/js/editDelete.js"></script>
